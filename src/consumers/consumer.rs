...
pub(super) struct Consumer<C: Send + Sync + 'static, E: Send + Sync + 'static> {
    ...
    handler: Arc<dyn Handler<Context = C, Error = E>>,
...
impl<C: Send + Sync + 'static, E: Send + Sync + 'static> Consumer<C, E> {
    ...
    pub(super) async fn new(
        ...
        let channel = transport_factory.get_channel().await?;
    ...
    pub(super) async fn run_until_shutdown(
        ...
        let mut shutdown = pin::pin!(shutdown.wait_for_signal().fuse());
        ...
        let result = 'event_loop: loop {
            ...
            tokio::select! {
                ...
                _ = shutdown.as_mut() => {
                    ...
                    let _ = self.channel.basic_cancel(consumer.tag().as_str(), <_>::default()).await;
                }
